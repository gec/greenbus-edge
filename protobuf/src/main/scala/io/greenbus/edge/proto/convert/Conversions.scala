...
object ValueConversions {
  ...
  def fromProto(msg: proto.Value): Either[String, edge.Value] = {
    ...
    msg.getValueTypesCase match {
      ...
      case ValueTypesCase.PATH_VALUE => Conversions.fromProto(msg.getPathValue).map(r => edge.ValuePath(r))
      case ValueTypesCase.ENDPOINT_PATH_VALUE => Conversions.fromProto(msg.getEndpointPathValue).map(r => edge.ValueEndpointPath(r))
  ...
  def toProto(obj: edge.Value): proto.Value = {
    obj match {
      ...
      case v: edge.ValuePath => proto.Value.newBuilder().setPathValue(Conversions.toProto(v.v)).build()
      case v: edge.ValueEndpointPath => proto.Value.newBuilder().setEndpointPathValue(Conversions.toProto(v.v)).build()
  ...
  def fromProto(msg: proto.IndexableValue): Either[String, edge.IndexableValue] = {
    ...
    msg.getValueTypesCase match {
      ...
      case ValueTypesCase.SINT64_VALUE => Right(edge.ValueInt64(msg.getSint64Value))
      case ValueTypesCase.UINT64_VALUE => Right(edge.ValueUInt64(msg.getUint64Value))
      case ValueTypesCase.STRING_VALUE => Right(edge.ValueSimpleString(msg.getStringValue))
      case ValueTypesCase.BYTES_VALUE => Right(edge.ValueSimpleBytes(msg.getBytesValue.toByteArray))
      case ValueTypesCase.UUID_VALUE => Right(edge.ValueUuid(fromProtoSimple(msg.getUuidValue)))
      case ValueTypesCase.VALUETYPES_NOT_SET => Left("Unrecognizable value type")
      case _ => Left("Unrecognizable value type")
    }
  ...
  def toProto(obj: edge.IndexableValue): proto.IndexableValue = {
    obj match {
      ...
      case edge.ValueSimpleString(v) => proto.IndexableValue.newBuilder().setStringValue(v).build()
      case edge.ValueSimpleBytes(v) => proto.IndexableValue.newBuilder().setBytesValue(ByteString.copyFrom(v)).build()
