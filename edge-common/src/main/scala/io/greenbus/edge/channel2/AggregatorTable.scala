...
object AggregatorTable {

  def forBuilder[Key, Handler <: Aggregator](buildHandler: Key => Handler): AggregatorTable[Key, Handler] = {
    new AggregatorTableImpl(buildHandler)
  }

  class AggregatorTableImpl[Key, Handler <: Aggregator](buildHandler: Key => Handler) extends AggregatorTable[Key, Handler] {

    private var pending = Map.empty[Key, Handler]

    def handleForKey(key: Key, closeable: CloseableComponent)(set: Handler => Unit): Unit = {
      closeable.onClose.subscribe(() => onClose(key))
