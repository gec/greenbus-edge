...
class ClientSenderChannelImpl[A](
  ...
  s: Sender,
  serializer: A => (Array[Byte], Int),
  promise: Promise[TransferChannelSender[A, Boolean]],
  parent: ResourceRemoveObserver)
...
abstract class BaseSenderChannelImpl[A](
  ...
  startOpened: Boolean,
  parent: ResourceRemoveObserver)
    extends CloseableChannel(ioThread, startOpened, parent, s) with TransferChannelSender[A, Boolean] with HandlerResource {

  private val deliverySequencer = new DeliverySequencer()

  private val sndImpl: channel.Sender[A, Boolean] = new channel.Sender[A, Boolean] {
    def send(obj: A): Future[Boolean] = {
      ...
      try {
        ...
        ioThread.marshal {
          ...
          delivery.setContext(new SentTransferDeliveryContext(prom))
  ...
  def sender: channel.Sender[A, Boolean] = sndImpl
...
class ClientReceiverChannelImpl[A](
  ...
  s: Receiver,
  deserializer: Array[Byte] => Option[A],
  promise: Promise[TransferChannelReceiver[A, Boolean]],
  parent: ResourceRemoveObserver)
...
abstract class BaseReceiverChannelImpl[A](
  ...
  startOpened: Boolean,
  parent: ResourceRemoveObserver)
    extends CloseableChannel(ioThread, startOpened, parent, s) with TransferChannelReceiver[A, Boolean] with LazyLogging {

  private val rcvImpl = new QueueingReceiverImpl[A, Boolean](ioThread)

  def receiver: channel.Receiver[A, Boolean] = rcvImpl
  ...
  val handler: ReceiverContext = new ReceiverContext {
    def onDelivery(r: Receiver, d: Delivery): Unit = {
      ...
      if (d.isReadable && !d.isPartial) {
        ...
        try {
          deserializer(buffer) match {
            ...
            case Some(obj) => {
              ...
              rcvImpl.handle(obj, promise)
              val fut = promise.future

              import scala.concurrent.ExecutionContext.Implicits.global

              fut.onSuccess {
                case true => ioThread.marshal { d.settle() }
                case false => ioThread.marshal { d.settle() } // TODO: distinguish delivery state?
              }
              fut.onFailure {
                case ex: Throwable => logger.warn("Delivery acknowledgment failed")
              }
            }
          }
        ...
        // TODO: do this when we go past a threshold rather than a packet every time
        r.flow(pending)
      }
    }
...
abstract class CloseableChannel(ioThread: CallMarshaller, startOpen: Boolean, parent: ResourceRemoveObserver, link: Link) extends Closeable with HandlerResource {

  private var opened = startOpen
  private var userClosed = false
  private val closeSink = new SimpleLatchSink(handleUserClose)
  protected val closeSource = new LocallyAppliedLatchSource(ioThread)
